		...
		if ( ! empty( $related ) ) {
			...
			<div class="ml-relatedposts-list">
				...
				foreach ( $related as $item ) {
					...
					?>
					<a class="ml-relatedposts-a" onclick="nativeFunctions.handlePost(<?php echo intval( $related_id ); ?>);">
...
if ( ! function_exists( 'ml_related_posts_fallback' ) ) {
	...
	function ml_related_posts_fallback( $post_id ) {
		...
		if ( count( $results ) < $size ) {
			ml_add_related_posts_using_args( $results, $args, $post_id );
		}
		return $results;
	}
		...
		if ( $data->post_count ) {
			...
			*/
			...
			foreach ( $data->posts as $post ) {
				if ( ( $current_post_id !== $post->ID ) && ( count( $results ) < $size ) ) {
