...
<script type="text/javascript" data-cfasync="false">
	...
	var loaded = mlPostsData['posts'].length - ( mlPostsData['ad_count'] || 0 ); // do not count Ads.
	var rendered = 0;
	...
	document.addEventListener( "DOMContentLoaded", function( event ) {
		...
		page.onInfiniteScroll = function (done) {
			if (!noMorePosts) {
				noMorePosts = true;
				getNewArticles(siteURL, loaded, <?php echo wp_json_encode( sanitize_text_field( wp_unslash( $_SERVER['QUERY_STRING'] ) ) ); ?>).then(function (more) {
					if (more === 0) {
						noMorePosts = true;
					} else {
						noMorePosts = false;
