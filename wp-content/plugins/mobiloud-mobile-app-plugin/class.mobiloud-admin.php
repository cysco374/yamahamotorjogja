...
class Mobiloud_Admin {
	...
	public static function menu_get_init() {
		...
		if ( $active_step === self::$welcome_steps[4] ) {
					...
					if ( ! empty( $_POST['ml_tabbed_navigation'] ) ) {
						...
						foreach ( $tn_data['tabs'] as $i => $_tab ) {
							...
							switch ( $_tab['type'] ) {
								...
								case 'list':
									$endpoint                                  = '?taxonomy=' . $_tab['taxonomy_type'];
									$endpoint                                 .= '&term_id=' . $_tab['taxonomy_id'];
									$endpoint                                 .= '&orderby=' . $_tab['taxonomy_orderby'];
									$endpoint                                 .= '&order=' . $_tab['taxonomy_order'];
									$tn_data['tabs'][ $i ]['endpoint_url']     = $base_endpoint_url . $endpoint;
									$tn_data['tabs'][ $i ]['endpoint_url_web'] = $base_endpoint_url_web . $endpoint;
	...
	public static function ajax_welcome() {
		if ( Mobiloud::is_action_allowed_ajax( 'tab_welcome', false ) ) {
			...
			$message    = ! empty( $_POST['ml_message'] ) ? sanitize_textarea_field( $_POST['ml_message'] ) : '';
			...
			Mobiloud::set_option( 'ml_user_site', $site );
			Mobiloud::set_option( 'ml_user_company', $company );
			Mobiloud::set_option( 'ml_user_phone', $phone );
			Mobiloud::set_option( 'ml_user_message', $message );
			...
			$params = array(
				...
				'code_initial' => (string) Mobiloud::get_option( 'ml_code', '' ),
				// 'ip'           => $ip,
			...
			if ( ! is_wp_error( $result ) && ( '' != wp_remote_retrieve_body( $result ) ) ) {
				...
				if ( is_array( $answer ) && ! empty( $answer['success'] ) && is_array( $answer['data'] ) ) {
					...
					$next_step = admin_url( 'admin.php?page=mobiloud&step=' . self::$welcome_steps[2] );
	...
	*/
	 ...
	 */
	private static function update_menu_with_items( $menu_name, $category_ids, $page_ids, $with_subcategories = false ) {
