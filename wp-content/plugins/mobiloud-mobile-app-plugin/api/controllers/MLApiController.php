...
class MLApiController {
	...
	public function prepare_response( MLQuery $ml_query, $return_array ) {
		...
		// insert List Ads?
		if ( 'web' === Mobiloud::get_option( 'ml_list_type' ) && Mobiloud::get_option( 'ml_list_ads_enabled' ) ) { // only for 'web' mode.
	 ...
	 * @param mixed   $json_string
	 */
	public function store_cache_response( $ml_query, $json_string ) {
		if ( $this->response_cache_on ) {
			$key      = http_build_query( $ml_query->ml_request );
			$ml_cache = new MLCacheController();
